...
macro_rules! def_operation {
    ($name:ident, $operation_name:expr) => {
        #[derive(Clone, Copy)]
        pub struct $name(circt_sys::capi::MlirOperation);

        impl WrapRaw for $name {
            type RawType = circt_sys::capi::MlirOperation;
            fn from_raw(raw: circt_sys::capi::MlirOperation) -> Self {
                Self(raw)
            }
            fn raw(&self) -> circt_sys::capi::MlirOperation {
                self.0
            }
        }
